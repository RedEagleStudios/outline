...
Large media/table documents avoid repeated dropdown-definition scans through a document-identity `WeakMap`. Temporary upload, dimension-probe, and image-download object URLs are deterministically revoked, legacy upload placeholder React roots are unmounted, and table NodeViews clean up animation frames and scroll listeners deterministically. Real React editor images use native lazy loading and asynchronous decoding; data-URI placeholders, ProseMirror attributes and schema serialization, persisted content, Markdown, server HTML, and exports remain unchanged and eager.
...
Current table NodeViews register with one measurement coordinator per browser document. A pooled IntersectionObserver limits top-level sticky-header work to nearby tables; eligible tables share one capture-phase document scroll listener and one read-all-before-write-all animation frame. Hidden pages cancel queued geometry work and re-observe tables on resume instead of flushing stale measurements. Nested tables retain initial and local horizontal geometry updates without sticky measurements or document-scroll participation. Missing observer or containment support fails open to the previous rendering behavior. Focused tests cover observer entry/exit, hidden/resume, nested tables, local-scroll isolation, cleanup, and fail-open invariants; genuine browser hidden/resume performance remains manual rollout evidence and is not claimed from JSDOM.

Dropdown identity repair now performs one initial invariant audit, skips non-document and safely unrelated transactions, and uses an identity-preserving fast path for normal option selection. Relevant or ambiguous changes—including broad collaboration replacements—conservatively retain a full final-document scan and the existing first-in-document duplicate ownership semantics. Repair transactions are marked to prevent a second append round. Focused tests cover malformed initial content, metadata and option changes, duplicate and legacy repair, multi-step and broad replacements, ownership ordering, and bounded recursion.

Eligible mounted top-level table NodeViews now progressively use runtime-only `content-visibility` containment only after IntersectionObserver confirms that they are outside the generous root margin, with a row-count-derived initial intrinsic block-size estimate and browser remembered sizing. Unknown and nearby tables remain uncontained so paint containment cannot clip the interactive row-selection and add-row gutters extending beyond the table wrapper; containment is removed immediately on near entry and restored on far exit. The managed class and estimate never enter ProseMirror schema, static, server, or export HTML; nested tables, unavailable observer/CSS paths, and unsupported browsers retain existing behavior. Print styles force managed tables visible and remove containment, sticky transforms, overflow clipping, and scroll shadows so all rows remain printable. JSDOM tests cover runtime eligibility, observer transitions, estimates, serialization boundaries, updates, and cleanup, but do not validate browser layout or print rendering.

Full-width tables are excluded from runtime rendering containment because `content-visibility` constrains their transformed, viewport-spanning layout to the normal document width and crops the scrollable content. Layout changes remove or restore containment immediately without remounting; the shared near-viewport measurement coordinator remains active for full-width tables.

Document and template print actions now prepare the target browser document before invoking print. Preparation activates managed tables and temporarily changes mounted lazy editor images to eager, waits for load/error settlement and best-effort decode, and uses a bounded overall timeout before restoring the React-only loading policy. Editor lifecycle registration provides a ref-counted synchronous beforeprint/afterprint fallback for direct browser printing, where completeness remains best-effort. Application print preparation does not affect eager schema/server/export serialization, and all attributes and runtime leases are restored after application printing, errors, afterprint, or final editor unmount without changing document data or schema output.
