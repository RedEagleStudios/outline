...
Eligible mounted top-level table NodeViews now progressively use runtime-only `content-visibility` containment with a row-count-derived initial intrinsic block-size estimate and browser remembered sizing. The managed class and estimate never enter ProseMirror schema, static, server, or export HTML; nested tables, unavailable observer/CSS paths, and unsupported browsers retain existing behavior. Print styles force managed tables visible and remove containment, sticky transforms, overflow clipping, and scroll shadows so all rows remain printable. JSDOM tests cover runtime eligibility, estimates, serialization boundaries, updates, and cleanup, but do not validate browser layout or print rendering.
