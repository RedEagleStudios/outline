...
### Default-Off Frame Viewport Lifecycle Integration

Production iframe gating has two independent controls that both default false: public `VIEWPORT_GATED_EMBEDS_ENABLED=false` and `TeamPreference.ViewportGatedEmbeds=false`. No migration or settings UI was added. The effective policy requires the primary current-document surface to be eligible, both controls to be true, and no public share. Revisions, presentation, overview, comments, previews, templates, the cache-only Multiplayer Editor, and public shares remain ineligible.

Team false propagates live through existing WebSocket-backed team model updates; global false is page-load scoped. Live disable removes viewport work and restores legacy mounting while preserving existing iframe identity. A false-to-true transition does not retrofit existing NodeViews and applies only to newly created or explicitly remounted Embed NodeViews. All 14 production custom services that own a `Frame` explicitly forward only allowlisted lifecycle props alongside service-specific frame props and refs. Native video, PDF, images, Drive links, and other non-Frame resources remain excluded.
...
Local production-surface validation passed the default-off baseline, canary-on initialization after the constructor fix, two-client WebSocket live rollback, iframe identity retention, no navigation, exactly one sanitized `live_disable` summary, public-share exclusion, and one SPA-unmount summary. Controls were restored false afterward. No production, Railway, Outline MCP, or deployment access occurred.

Production activation remains prohibited pending manual headed-browser evidence in an environment that genuinely reports a hidden page for more than 31 seconds and positive natural `pagehide` delivery to a verified operational analytics sink, followed by explicit rollout approval.
